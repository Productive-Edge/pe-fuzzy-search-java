...
class Bitap65v2Plus extends BaseBitap {

    private final Char2ObjectOpenHashMap<BitVector> positionBitMasks;

    private final int patternLength;

    Bitap65v2Plus(CharSequence pattern, int maxLevenshteinDistance) {
        this(pattern, maxLevenshteinDistance, false);
    }

    Bitap65v2Plus(CharSequence pattern, int maxLevenshteinDistance, boolean caseInsensitive) {
    ...
    final class Matcher extends BaseBitap.Matcher {
        ...
        private final BitVector substitution = new BitVector(Bitap65v2Plus.this.patternLength);
        private final BitVector insertion = new BitVector(Bitap65v2Plus.this.patternLength);
        private final BitVector matching = new BitVector(Bitap65v2Plus.this.patternLength);
        private final BitVector testDeletion = new BitVector(Bitap65v2Plus.this.patternLength);
        ...
        public boolean testNextSymbol() {
            final BitVector charPositions = Bitap65v2Plus.this.positionBitMasks.getOrDefault(super.text.charAt(super.index), null);
            swapMatching();
                    ...
                    if (this.insertion.lessThan(this.substitution) && this.insertion.lessThan(this.matching)) {
                        ...
                        if (found) {
                            ...
                            if (nextIndex < super.maxIndex) {
                                final BitVector nextCharPositions = Bitap65v2Plus.this.positionBitMasks.get(super.text.charAt(nextIndex));
