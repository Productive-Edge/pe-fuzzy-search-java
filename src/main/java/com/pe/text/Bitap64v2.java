...
class Bitap64v2 extends BaseBitap {

    private final Char2LongOpenHashMap positionBitMasks;
    private final long lastBitMask;

    Bitap64v2(CharSequence pattern, int maxLevenshteinDistance) {
        this(pattern, maxLevenshteinDistance, false);
    }

    Bitap64v2(CharSequence pattern, int maxLevenshteinDistance, boolean caseInsensitive) {
    ...
    final class Matcher extends BaseBitap.Matcher {
        ...
        public boolean testNextSymbol() {
            final long charPositions = Bitap64v2.this.positionBitMasks.getOrDefault(super.text.charAt(super.index), -1L);
            swapMatching();
            super.levenshteinDistance = 0;
            this.currentMatchings[0] = (this.previousMatchings[0] << 1) | charPositions;
            if (0L == (this.currentMatchings[0] & Bitap64v2.this.lastBitMask)) {
                return true;
            }
...
//                final int insertion = currentMatchings[levenshteinDistance - 1] << 1; // original Bitap insert
                ...
                final boolean found = 0L == (this.currentMatchings[super.levenshteinDistance] & Bitap64v2.this.lastBitMask);
                if (current >= deletion) {
                    if (insertion < substitution && insertion < matching) {
                        ...
                        if (found) {
                            ...
                            if (nextIndex < super.maxIndex) {
                                final long nextCharPositions = Bitap64v2.this.positionBitMasks.getOrDefault(super.text.charAt(nextIndex), -1);
                                if ((nextCharPositions & Bitap64v2.this.lastBitMask) == 0L) {
        ...
        public boolean testNextInsert(final int iteration) {
            final long bitMask = Bitap64v2.this.lastBitMask >>> iteration;
