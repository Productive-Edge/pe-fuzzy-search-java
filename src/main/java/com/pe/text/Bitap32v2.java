...
class Bitap32v2 extends BaseBitap {

    private final Char2IntOpenHashMap positionBitMasks;
    private final int lastBitMask;

    public Bitap32v2(CharSequence pattern, int maxLevenshteinDistance) {
        this(pattern, maxLevenshteinDistance, false);
    }

    public Bitap32v2(CharSequence pattern, int maxLevenshteinDistance, boolean caseInsensitive) {
    ...
    final class Matcher extends BaseBitap.Matcher {
        ...
        public boolean testNextSymbol() {
            final int charPositions = Bitap32v2.this.positionBitMasks.getOrDefault(super.text.charAt(super.index), -1);
            swapMatching();
            super.levenshteinDistance = 0;
            this.currentMatchings[0] = (this.previousMatchings[0] << 1) | charPositions;
            if (0 == (this.currentMatchings[0] & Bitap32v2.this.lastBitMask)) {
                return true;
            }
...
//                final int insertion = currentMatchings[levenshteinDistance - 1] << 1; // original Bitap insert
                ...
                final boolean found = 0 == (this.currentMatchings[super.levenshteinDistance] & Bitap32v2.this.lastBitMask);
                if (current >= deletion) {
                    if (insertion < substitution && insertion < matching) {
                        ...
                        if (found) {
                            ...
                            if (nextIndex < super.maxIndex) {
                                final int nextCharPositions = Bitap32v2.this.positionBitMasks.getOrDefault(super.text.charAt(nextIndex), -1);
                                if ((nextCharPositions & Bitap32v2.this.lastBitMask) == 0) {
        ...
        public boolean testNextInsert(final int iteration) {
            final int bitMask = Bitap32v2.this.lastBitMask >>> iteration;
